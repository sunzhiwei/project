...
$(function() {
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			title : "额度更改历史",
			url : "${ctx}/hxLimit/getLimitHistoryPageList?limitId="+limitId,
			nowrap : false,
			striped : true,
			...
			columns : [ [ {
				...
				align:'center',
				formatter:function(value, row, index){ 
					if(row.limitOperateReason=='0'){
						return "初始化";
					}else if(row.limitOperateReason=='1'){
						return "手工修改";
					}
                }  
			}, {
		...
		<form id="form" method="post">
			...
			<table border=1 style="cellSpacing:1;cellPadding:3;width:100%;background-color:#eff5ff;border-collapse:collapse;">
				<tr>
					...
					<td width="25%"><label>${limit.limitCash}</label></td>
				</tr>
				<tr>
					<td width="25%" align="center">信用额度:</td>
					<td width="25%"><label>${limit.limitCredit}</label></td>
					<td width="25%" align="center">可用额度:</td>
					<td width="25%"><label>${limit.limitCash+limit.limitCredit}</label></td>
				</tr>
			</table>
		</form>
