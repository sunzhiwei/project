		...
		$('#projectGrid').datagrid({
			...
			columns : [ [ {
				...
				editor:{
					type:'combobox',
					options:{
						url:'${ctx}/installReceipt/getModel',
						panelHeight:'auto',
						onSelect:function(value){
							var currentRowNo = parseInt($(this).parents('tr[id^=datagrid-row-r1-2-]').attr("id").replace("datagrid-row-r1-2-",""));
							var gomeCode = $('#projectGrid').datagrid('getEditor', {index:currentRowNo,field:'gomeCode'});
							var brand = $('#projectGrid').datagrid('getEditor', {index:currentRowNo,field:'brand'});
							$(gomeCode.target).combobox('setValue', value.gomeCode);
							$(brand.target).combobox('setValue', value.brand);
						}
					}
				}
			},{field : 'gomeCode',title : '国美代码',hidden:'true',align :'center',width : 30,
				editor:{
					type:'combobox',
					options:{
						url:'${ctx}/hxCode/getCombobox/gmdm',
						panelHeight:'auto'
					}
				}
			}, {field : 'brand',title : '品牌',hidden:'true',align :'center',width : 30,
				editor:{
					type:'combobox',
					options:{
						url:'${ctx}/hxCode/getCombobox/pp',
						panelHeight:'auto'
					}
				}
...
<form id="formCustomer" method="post">
	...
	<table border=1 style="cellSpacing:1;cellPadding:3;width:100%;background-color:#eff5ff;border-collapse:collapse">
		<tr>
			...
			<td width="20%">性别</td>
			<td width="30%">
				<input name="customerSex" class="easyui-combobox" data-options="url:'${ctx}/hxCode/getCombobox/xb',panelHeight:'auto', editable:false"/>
			</td>
		</tr>
		<tr>
			<td>会员卡号</td>
			...
			<td><input name="province" type="text" class="easyui-combobox" data-options="url:'${ctx}/hxCode/getCombobox/csf',panelHeight:'100',editable:true,required:true"></td>
		</tr>
		<tr>
			<td>区号</td>
...
<form id="formProduct" method="post">
	...
	<table border=1 style="cellSpacing:1;cellPadding:3;width:100%;background-color:#eff5ff;border-collapse:collapse">
		<tr>
			...
			<td width="30%"><input name="installUnit" type="text" class="easyui-combobox" data-options="url:'${ctx}/installReceipt/getServices',required:true"></td>
		</tr>
		<tr>
			<td>销售门店<font color="red">*</font></td>
			<td><input name="saleMarket" type="text" class="easyui-combobox" data-options="url:'${ctx}/installReceipt/getStore',required:true"></td>
			<td>安装省份<font color="red">*</font> </td>
			<td><input name="installProvince" type="text" class="easyui-combobox" data-options="url:'${ctx}/hxCode/getCombobox/csf',panelHeight:'100',editable:true,required:true"></td>
		</tr>
		<tr>
			<td>安装详细地址<font color="red">*</font></td>
			<td colspan="3"><textarea name="installDetailAddress" data-options="required:true" rows="2" cols="80">${customerProduct.lxdz}</textarea></td>
		</tr>
		<tr>
...
<form id="formTicket" method="post">
	...
	<table border=1 style="cellSpacing:1;cellPadding:3;width:100%;background-color:#eff5ff;border-collapse:collapse">
		...
		<tr>
			...
			<td><input name="serviceUnit" type="text" class="easyui-combobox" data-options="url:'${ctx}/installReceipt/getServiceUnit?wd=${usr.companyId}',required:true"></td>
			<td></td>
			<td></td>
		</tr>
		<tr>
