...
// 				searchs();
	...
	$('#receiveUnit').combobox({
		...
		onSelect : function(record) {
			$.post(
					'${ctx}/ParcelDelivery/getOrgnazationBySuit?receiveUnit='
							+ record.value, {}, function(returnObj) {
						$(" input.receiveName").vla(
								returnObj.orgManager);
						$(" input.receivePhone").vla(
								returnObj.orgReference);
						alert(org_reference);
						$(" input.receivePost").vla(
								returnObj.orgPostCode);
						$(" input.receiveAddress").vla(
								returnObj.orgAddress);
					});
		}
	});
				...
				<table class="table table-hover table-condensed" width="100%;">
					<tr>
						...
						<td><input id="receiveUnit" name="receiveUnit"
							...
							data-options="url:'${ctx}/hxCode/getOrgCombobox?value=${parcel.receiveUnit}'" />
