...
<script type="text/javascript">
	...
	function onStart(heigh){
	 	dataGrid = $('#dataGrid').datagrid({
			title : "安装服务单",
			url : "${ctx}/hxConfirmSettlement/getConfirmSettlementPageList",
			striped : true,
            height : heigh,
            collapsible : true,
            autoRowHeight : false,
			...
			columns : [ [ 
			   {field : 'service_unit',title : '服务单位', width : 10 ,align:'center'
				...
				formatter : function(value, row, index) {
					if(row.settlement_status=="S1"){
						return '<a href="#" onclick="examine(\'' + row.settlement_id + '\');">审核</a> | <a href="#" onclick="update(\'' + row.settlement_id + '\');">修改</a> ';
					}else{
						return '<a href="#" onclick="checkOut(\'' + row.settlement_id + '\');">查看</a>';
					}
				}
			} ] ],
			toolbar : '#toolbar',
	...
	function examine(settlementId){
		window.location.href="${ctx}/hxConfirmSettlement/examineSettlementShow/"+settlementId;
	}
	
	function update(settlementId){
		window.location.href="${ctx}/hxConfirmSettlement/confirmSettlementUpdateView/"+settlementId;
	}
	
	function checkOut(settlementId){
		window.location.href="${ctx}/hxConfirmSettlement/examineSettlementDetail/"+settlementId;
	}
	
	function test(){
		window.location.href="${ctx}/hxExamineSettlement/goTest";
	}
	...
	<div class="easyui-layout" data-options="fit:true,border:false">
		<div data-options="region:'north',title:'查询条件',border:false,collapsible:true,onBeforeCollapse:function(){clos();},onBeforeOpen:function(){ope();}" style="height: 118px; overflow: hidden;">
			<form id="searchForm">
				<table class="table table-hover table-condensed" width="100%;"
					...
					<tr>
						<td width="5%">服务单位</td>
						<td width="15%"><input name="serviceUnit" placeholder="请输入查询条件" class="easyui-combobox" data-options="url:'${ctx}/installReceipt/getStore',panelHeight:'100',editable:true "></td>
		...
		<div class="easyui-layout" data-options="fit:true,border:false">
			<div id="toolbar" style="display: "";">
				...
				<a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-excel',plain:true" onclick="test();">test</a>
			</div>
		</div>
