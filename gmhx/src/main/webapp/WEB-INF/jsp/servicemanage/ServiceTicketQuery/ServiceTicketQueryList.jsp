...
<script type="text/javascript">
    ...
    function onStart(heigh){
        dataGrid = $('#dataGrid').datagrid({
            ...
            },{field : 'voucher_status',title : '单据<br/>状态',width : 10,align:'center'
    ...
    function exportExcel() {
        ...
        window.location.href = "${ctx}/serviceTicketQuery/exportExcel?" + 
        encodeURI($('#searchForm').serialize()) + "&tableField=" + fieldArray.join("|") +                 
        "&tableHeader=" + encodeURI(encodeURI(titleArray.join("|")));
    }
...
<body>
    <div class="easyui-layout" data-options="fit:true,border:false">
        <div data-options="region:'north',title:'查询条件',border:false,collapsible:true,onBeforeCollapse:function(){clos();},onExpand:function(){ope();}"  style="height: 340px; overflow: hidden;">
            <form id="searchForm">
            	<table style="width:100%; padding: 10px 10px 10px 10px;">
            		<tr>
            			<td width="5%">安装日期:</td>
                       	<td width="15%">
                        	<input id="ksrq" size="11" placeholder="起始安装日期" class="Wdate" name="mod_installDate_st" type="text" readonly="readonly" onFocus="WdatePicker({maxDate:'#F{$dp.$D(\'jsrq\')}'})"/>
							<input id="jsrq" size="11" placeholder="结束安装日期" class="Wdate" name="mod_installDate_end" type="text" readonly="readonly" onFocus="WdatePicker({minDate:'#F{$dp.$D(\'ksrq\')||\'%y-%M-%d\'}'})"/> 
                       	</td>
                       	<td width="5%">销售日期:</td>
                        <td width="15%">
                        	<input id="ksrq" size="11" placeholder="起始销售日期" class="Wdate" name="mod_saleDate_st" type="text" readonly="readonly" onFocus="WdatePicker({maxDate:'#F{$dp.$D(\'jsrq\')}'})"/>
							<input id="jsrq" size="11" placeholder="结束销售日期" class="Wdate" name="mod_saleDate_end" type="text" readonly="readonly" onFocus="WdatePicker({minDate:'#F{$dp.$D(\'ksrq\')||\'%y-%M-%d\'}'})"/>
                        </td>
                        <td width="5%">服务日期:</td>
                        <td width="15%">
                        	<input id="ksrq" size="11" placeholder="起始服务日期" class="Wdate" name="mod_serviceDate_st" type="text" readonly="readonly" onFocus="WdatePicker({maxDate:'#F{$dp.$D(\'jsrq\')}'})"/>
							<input id="jsrq" size="11" placeholder="结束服务日期" class="Wdate" name="mod_serviceDate_end" type="text" readonly="readonly" onFocus="WdatePicker({minDate:'#F{$dp.$D(\'ksrq\')||\'%y-%M-%d\'}'})"/>
                        </td>
                        <td width="5%">审核时间:</td>
                        <td width="15%">
                       		<input id="ksrq" size="11" placeholder="起始审核时间" class="Wdate" name="mod_checkDate_st" type="text" readonly="readonly" onFocus="WdatePicker({maxDate:'#F{$dp.$D(\'jsrq\')}'})"/>
							<input id="jsrq" size="11" placeholder="结束审核时间" class="Wdate" name="mod_checkDate_end" type="text" readonly="readonly" onFocus="WdatePicker({minDate:'#F{$dp.$D(\'ksrq\')||\'%y-%M-%d\'}'})"/>
                        </td>
                        <td width="5%">创建时间:</td>
                        <td width="15%">
                        	<input id="ksrq" size="11" placeholder="起始创建时间" class="Wdate" name="mod_createDate_st" type="text" readonly="readonly" onFocus="WdatePicker({maxDate:'#F{$dp.$D(\'jsrq\')}'})"/>
							<input id="jsrq" size="11" placeholder="结束创建时间" class="Wdate" name="mod_createDate_end" type="text" readonly="readonly" onFocus="WdatePicker({minDate:'#F{$dp.$D(\'ksrq\')||\'%y-%M-%d\'}'})"/>
                        </td>
            		</tr>
                    <tr>
