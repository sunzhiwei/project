...
<script type="text/javascript">
	...
	function importExcel() {
		...
		parent.$.modalDialog({
			...
			height :250,
			closable : false,
			href : '${ctx}/common/import/importData?templateName=goodbill&actionName=importHxGoodbill',
			buttons : [ 
			{
				text : '关闭',
				handler : function() {
	...
	function show(gsxx, thdh, xslx) {
		var key = gsxx+","+thdh+","+xslx;
		window.location.href = "${ctx}/hxBillNumImportQuery/showView/"+ key;
	}
	function exportExcel() {
		...
		window.location.href = "${ctx}/hxBillNumImportQuery/exportExcel?" + encodeURI($('#searchForm').serialize()) + "&tableField=" + fieldArray.join("|") + "&tableHeader=" + encodeURI(encodeURI(titleArray.join("|")));
	}
</script>
</head>
<body>
		...
		<div data-options="region:'north',title:'查询条件',border:false,collapsible:false" style="height: 210px; overflow: hidden;">
			<form id="searchForm">
				<table class="table table-hover table-condensed" style="width:100%; padding: 7px 80px 0px 80px">
					<tr>
						<td width="10%"></td>
						<td width="10%">CRM分部</td>
						<td width="5%"><input name="xx" placeholder="输入查询条件" editable="false" class="easyui-combobox" data-options="url:'${ctx}/hxCode/getCombobox/fb'"/></td>
						<td width="10%"></td>
						<td width="10%">分部名称</td>
					...
					<tr>
						...
						<td width="10%"></td>
						<td width="10%">数量</td>
						<td width="5%"><input name="shuliang" placeholder="输入查询条件" class="span2" /></td>
						<td width="20%"></td>
